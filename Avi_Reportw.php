...
//	frmRecEntry.Flexi_Allow.value   = items[7];

	frmRecEntry.Salary_Total.value  = items[8];
	
	frmRecEntry.Pro_Tax.value       = items[9];

	frmRecEntry.PF.value            = items[10];

	frmRecEntry.IT.value            = items[11];

	frmRecEntry.Others.value        = items[12];

	frmRecEntry.Deduct_Total.value  = items[13];

	frmRecEntry.Net_Salary.value    = items[14];

	frmRecEntry.Gross_Salary.value  = items[15];

	frmRecEntry.Advance.value       = items[16];
	
	frmRecEntry.ESI.value 		= items[17];
		
	frmRecEntry.Rupees.value        = items[18];
...
//	frmRecEntry.Flexi_Allow.value   = items[10];
	...
	frmRecEntry.Advance.value       = items[19];
	
	frmRecEntry.ESI.value 		= items[20];
		
	frmRecEntry.Rupees.value        = items[21];
...
function calculateSalary()
{
	...
	var Basic,DA,HRA,CCA,Salary_Total=0,Pro_Tax=0,ESI,PF=0;
	var TempTot = 0;
	var Deduct_Total,Net_Salary,Rupees,Flexi_Allow;
	var Total_Sal;
...
*/
	...
	if((Gross <= 9999))
	{
		ESI = Math.ceil(ESal * 0.0175);
		CmpESI_Share = Math.ceil(ESal * 0.0475);
	...
	else if(ESal >=10000 && ESal <=14999)
	{

		Pro_Tax = 150;
	}
	else if(ESal >=8000 && ESal <=9999)
	{
		Pro_Tax = 100;
	}	
	else if(ESal >=5000 && ESal <=7999)
	{
		Pro_Tax = 60;
	}
	else if(ESal >=3000 && ESal <=4999)
	{
		Pro_Tax = 30;
	}
	else if(ESal<3000)
	...
	if(((Basic + DA) <= 6500) && ((Basic + DA) >= 2843))
	{
		PF = Math.ceil((Basic + DA) * 0.12);
		frmRecEntry.PF.value = PF;
		frmRecEntry.PF.readonly = true;
	}
	else
	{
		PF = parseFloat(frmRecEntry.PF.value);
	}
//	frmRecEntry.PF.value = PF;

	Total_Deduction = Math.ceil(IT + Others + PF + Pro_Tax + Advance + ESI);
	
	Net_Salary = Math.ceil(Total_Sal - Total_Deduction);

/*	if(Gross <= 4999)
	{
		frmRecEntry.Earned_Salary.value = ESal;
		frmRecEntry.Deduct_Total.value = Total_Deduction;
		frmRecEntry.Salary_Total.value = Total_Sal;
...
//if(!isNaN(parseFloat(document.frmRecEntry.Net_Salary)))
...
function onload_calculateSalary()
{
	...
	var Basic,DA,HRA,CCA,Salary_Total,Pro_Tax,ESI,PF=0;
	var Deduct_Total,Net_Salary,Rupees,Flexi_Allow;
...
<div id ='DivObj' name = 'DivObj' style = "top:0;left:0;height:0;width:0;display:'None';">
   ...
   <table border="1" cellspacing="0" width="265" bordercolorlight="#004848" bordercolordark="#FFFFFF" height="30">
    <tr>
    	...
    	<td class=header width="71%" align="right" bgcolor="#E0E0E0">
      	<p align="left"><b><font color="#000000">Employee Details</font></b></td>
              ...
              <ul style="color: #D99548" type="square">
                <li><a class = header href = "Mel_Report.php"><font color="#000000">MSPL</font></a></li>
              </ul>
            </td>
	...
	<tr>
		...
		<?php
			...
			$query = "select * from DEPARTMENT where COMPANY = 'A' ";
...
<span id="category"  style="color:#FF3399;font-weight:900;"></span>
	...
	<tr>
		...
		<option value='Select Employee'>Select Employee Name..</option></select></td>
		...
		<option value='Select Employee ID'>Select Employee ID....&nbsp;&nbsp;&nbsp;</option></select></td>
	</tr>
    ...
    <td width="72%" height="76" align="right">
      <table class=bodytext border="0" cellpadding="5" cellspacing="0" width="100%">
        <tr>
          ...
          <td width="15%" bgcolor="#EEEEEE" align="right">DA</td>
          <td width="37%" bgcolor="#EEEEEE"><input type="text" name="DA" size="20" readonly></td>
        </tr>
        <tr>
          <td width="24%" bgcolor="#EEEEEE" align="right">HRA</td>
          <td width="30%" bgcolor="#EEEEEE"><input type="text" name="HRA" size="20" readonly></td>
          <td width="15%" bgcolor="#EEEEEE" align="right">CCA</td>
          <td width="37%" bgcolor="#EEEEEE"><input type="text" name="CCA" size="20" readonly></td>
        </tr>
        <tr>
	...
	<td width="15%" bgcolor="#EEEEEE" align="right"></td>
    ...
    <td width="72%" height="95" align="right">
      <table class=bodytext border="0" cellpadding="5" cellspacing="0" width="100%">
        <tr>
    <td width="19%" height="19" align="right" bgcolor="#EEEEEE">P.Tax</td>
    ...
    <td width="22%" height="19" bgcolor="#EEEEEE"><input type="text" name="PF" size="20"></td>
        ...
        <tr>
    <td width="19%" height="19" align="right" bgcolor="#EEEEEE">IT</td>
    <td width="15%" height="19" bgcolor="#EEEEEE"><input type="text" name="IT" size="20"></td>
