...
<DIV ID = 'list'>
	...
	<table border="0" width="1500" cellspacing="1" cellpadding="4" align = 'center'>
              <tr class=header>
                ...
                <td width="250" bgcolor="#C0C0C0" valign="middle">
	...
	$Company = $_SESSION['company'];
		...
		while ($row = mysql_fetch_array($dept_result))
		{
			echo("<tr class = header>");
				...
				echo("<td width='70' colspan  = 26  bgcolor='#C0C0C0' align = 'center' valign='middle'>$row[0]</td>");
			echo("</tr>");

				$query = "select p.NAME,p.EMP_NO,e.DATE_OF_JOINING,i.AMOUNT,p.WORKING_DAYS,p.PRESENT_DAYS,p.BASIC,p.LTA,p.HRA,p.CCA,p.SPECIALALLOW,p.MR,p.EARNED_SALARY,p.PRO_TAX,p.PF,p.ESI,p.TDS,p.OTHERS,p.ADVANCE,p.DEDUCT_TOTAL,p.GROSS_SALARY,p.NET_SAL,e.ACNO,i.MONTH,i.YEAR from employee e,incrementdetails i, paydetails1 p where e.EMP_NO = p.EMP_NO AND p.EMP_NO = i.EMP_NO AND e.DEPT = '$row[0]' AND p.MONTH = '$cMon' AND p.YEAR = '$cYer' AND   p.COMPANY = '$Company'";
			...
			while ($rows = mysql_fetch_array($result))
			{
				...
				$MonName = str_pad($rows[23] , 2, "0", STR_PAD_LEFT);		
				...
				echo("<td width='200' bgcolor='#C0C0C0' >$rows[0]</td>");
				echo("<td width='200' bgcolor='#C0C0C0' >$rows[1]</td>");
				echo("<td width='200' bgcolor='#C0C0C0' >$Emp_DOJ</td>");
				if($rows[3] == 0)
				...
				if($rows[24] == 0)
					...
					echo("<td width='200' bgcolor='#C0C0C0' >NIL</td>");
					
				else
				
				echo("<td width='100' bgcolor='#C0C0C0' >01-${MonName}-${subyear}</td>");	
				...
				echo("<td width='70' bgcolor='#C0C0C0' >$rows[19]</td>");
				echo("<td width='70' bgcolor='#C0C0C0' >$rows[20]</td>");
				echo("<td width='70' bgcolor='#C0C0C0' >$rows[21]</td>");
				echo("<td width='70' bgcolor='#C0C0C0' >$rows[22]</td>");
			...
			{
				echo("<tr class = header>");
				echo("<td width='70' bgcolor='#C0C0C0' colspan  = 7  valign='middle'>Total</td>");
				if($rows[0] == "") $rows[0] = 0;
				...
				echo("<td width='70' bgcolor='#C0C0C0' valign='middle'>$rows[11]</td>");
				if($rows[12] == "") $rows[12] = 0;
				echo("<td width='70' bgcolor='#C0C0C0' valign='middle'>$rows[12]</td>");
				if($rows[13] == "") $rows[13] = 0;
				echo("<td width='70' bgcolor='#C0C0C0' valign='middle'>$rows[13]</td>");
				if($rows[14] == "") $rows[14] = 0;
				echo("<td width='70' bgcolor='#C0C0C0' valign='middle'>$rows[14]</td>");
				if($rows[15] == "") $rows[15] = 0;
				echo("<td width='70' bgcolor='#C0C0C0' valign='middle'>$rows[15]</td>");
				echo("<td width='70' bgcolor='#C0C0C0' valign='middle'>&nbsp;</td>");
